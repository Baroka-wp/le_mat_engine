...
def _generate_js_api_init(project: str) -> str:
    ...
    return (
        ...
        f"const _APP_DIR = {json.dumps(_app_dir)};\n"
        f"const _DB_PATH = {json.dumps(db_abs)};\n"
        ...
        "`;\n"
        f"  const url = 'http://127.0.0.1:' + _LEMAT_PORT + '/api/projects/' + _PROJECT + path;\n"
...
async def handle_user_api(project: str, route_path: str, request: Request):
    ...
    for file_path in api_files:
        ...
        if ext == ".py":
            command = ["python3", "-u", _API_RUNNER_PY]
        elif ext == ".js":
            command = ["node", _API_RUNNER_JS]
        else:
            continue
        ...
        try:
            ...
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(input=json.dumps(req_context).encode()),
                timeout=30.0,
            )
        except asyncio.TimeoutError:
            try:
