...
async def auth_middleware(request: Request, call_next):
    ...
    # User-defined API routes: allow X-API-Key header (validation happens in the handler)
    if "/api/" in path:
        parts = path.split("/")
        # Match /api/projects/{project}/api/{route}
        if len(parts) >= 5 and parts[1] == "api" and parts[2] == "projects" and parts[4] == "api":
...
active_processes: dict = {}  # run_id → (process, container_name_or_None)
...
# ── API runner paths ──
_API_RUNNER_PY = str(Path(__file__).resolve().parent / "api_runner_py.py")
_API_RUNNER_JS = str(Path(__file__).resolve().parent / "api_runner_js.js")
...
def _generate_python_lemat_init(project: str) -> str:
    project_dir = safe_path(project).resolve()
    db_path = find_db_file(project_dir, load_schema(project_dir))
    db_abs = str(db_path.resolve()) if db_path else None
    db_name = db_path.name if db_path else None
    # Resolve the directory where main.py / db_engine.py live
    _app_dir = str(Path(__file__).resolve().parent)
    return (
        "# _lemat_init.py — auto-generated by Le Mat\n"
        "import sys as _sys, os as _os\n"
        "# SDK dir: use env var (Docker) or hardcoded path (subprocess)\n"
        f'_SDK_DIR = _os.environ.get("LEMAT_SDK_DIR", {repr(_app_dir)})\n'
        ...
        "# Project dir: use CWD (works in both Docker and subprocess)\n"
        "_PROJECT_DIR = _Path(_os.getcwd())\n"
        f"_DB_NAME = {repr(db_name)}\n"
        f"_DB_PATH_DEFAULT = {repr(db_abs)}\n"
        f'_LEMAT_HOST = _os.environ.get("LEMAT_HOST", "127.0.0.1")\n'
        f'_LEMAT_PORT = int(_os.environ.get("LEMAT_PORT", "8000"))\n'
        f'_LEMAT_PROJECT = "{project}"\n'
        "\n"
        "# Resolve DB path: try container path first, then default, then glob\n"
        "_DB_PATH = None\n"
        "if _DB_NAME:\n"
        "    _candidate = _PROJECT_DIR / _DB_NAME\n"
        "    if _candidate.exists(): _DB_PATH = _candidate\n"
        "if not _DB_PATH and _DB_PATH_DEFAULT:\n"
        "    _candidate = _Path(_DB_PATH_DEFAULT)\n"
        "    if _candidate.exists(): _DB_PATH = _candidate\n"
        "\n"
        "def _get_db():\n"
        "    global _DB_PATH\n"
        ...
        "    url = f'http://{_LEMAT_HOST}:{_LEMAT_PORT}/api/projects/{proj_enc}' + path\n"
...
def _generate_js_lemat_init(project: str) -> str:
    return (
        ...
        "const _LEMAT_HOST = process.env.LEMAT_HOST || '127.0.0.1';\n"
        "const _LEMAT_PORT = parseInt(process.env.LEMAT_PORT || '8000', 10);\n"
        "function _api(method, path, body) {\n"
        "  return new Promise((res, rej) => {\n"
        "    const d = body ? JSON.stringify(body) : null;\n"
        "    const o = { method, hostname: _LEMAT_HOST, port: _LEMAT_PORT,\n"
...
def _generate_js_api_init(project: str) -> str:
    """Generate a JS init file for API runners that uses direct DB access via Python subprocess.
    This avoids HTTP deadlock when the API runner calls back to the server."""
    project_dir = safe_path(project).resolve()
    db_path = find_db_file(project_dir, load_schema(project_dir))
    db_abs = str(db_path.resolve()) if db_path else ""
    _app_dir = str(Path(__file__).resolve().parent)
    return (
        "// _lemat_api_init.js — auto-generated by Le Mat (direct DB access, no HTTP)\n"
        "const { execFileSync } = require('child_process');\n"
        "const _path = require('path');\n"
        f"const _APP_DIR = process.env.LEMAT_SDK_DIR || {json.dumps(_app_dir)};\n"
        f"const _DB_NAME = {json.dumps(db_path.name if db_path else None)};\n"
        f"const _DB_PATH_DEFAULT = {json.dumps(db_abs)};\n"
        "// Resolve DB: try project dir first, then default absolute path\n"
        "const _fs = require('fs');\n"
        "const _DB_PATH = (_DB_NAME && _fs.existsSync(_path.join(process.cwd(), _DB_NAME)))\n"
        "  ? _path.join(process.cwd(), _DB_NAME)\n"
        "  : (_DB_PATH_DEFAULT || '');\n"
        f"const _PROJECT = {json.dumps(project)};\n"
        "const _LEMAT_HOST = process.env.LEMAT_HOST || '127.0.0.1';\n"
        "const _LEMAT_PORT = parseInt(process.env.LEMAT_PORT || '8000', 10);\n"
        "\n"
        "function _dbQuery(op, args) {\n"
        "  const script = `\n"
        "import sys, json; sys.path.insert(0, ${JSON.stringify(_APP_DIR)})\n"
        "import db_engine; from pathlib import Path\n"
        "req = json.loads(sys.stdin.read())\n"
        "db = Path(req['db'])\n"
        "op = req['op']\n"
        "if op == 'all':\n"
        "    r = db_engine.select_all(db, req['table'], req.get('filters'), req.get('limit',1000), req.get('offset',0))\n"
        "elif op == 'find':\n"
        "    pk_col = db_engine.get_pk_col(db, req['table'])\n"
        "    r = db_engine.select_one(db, req['table'], pk_col, req['pk'])\n"
        "elif op == 'create':\n"
        "    r = db_engine.insert(db, req['table'], req['data'])\n"
        "elif op == 'update':\n"
        "    pk_col = db_engine.get_pk_col(db, req['table'])\n"
        "    r = db_engine.update(db, req['table'], pk_col, req['pk'], req['data'])\n"
        "elif op == 'delete':\n"
        "    pk_col = db_engine.get_pk_col(db, req['table'])\n"
        "    r = db_engine.delete(db, req['table'], pk_col, req['pk'])\n"
        "else:\n"
        "    r = {'error': 'unknown op'}\n"
        "print(json.dumps(r))\n"
        "`;\n"
        "  const input = JSON.stringify({ db: _DB_PATH, ...args, op });\n"
        ...
        "}\n"
        "\n"
        "// HTTP helper for mail (non-blocking OK via sync HTTP)\n"
        "function _httpSync(method, path, body) {\n"
        "  const http = require('http');\n"
        "  const script = `\n"
        "import urllib.request, json, sys\n"
        "req_data = json.loads(sys.stdin.read())\n"
        "data = json.dumps(req_data.get('body')).encode() if req_data.get('body') else None\n"
        "req = urllib.request.Request(req_data['url'], data=data,\n"
        "    headers={'Content-Type':'application/json'}, method=req_data['method'])\n"
        "try:\n"
        ...
        "`;\n"
        f"  const url = 'http://' + _LEMAT_HOST + ':' + _LEMAT_PORT + '/api/projects/' + _PROJECT + path;\n"
        ...
        "global.lemat = {\n"
        "  db: {\n"
        "    all:    (table, filters) => _dbQuery('all', { table, filters }),\n"
        "    find:   (table, pk)     => _dbQuery('find', { table, pk }),\n"
        "    create: (table, data)   => _dbQuery('create', { table, data }),\n"
        "    update: (table, pk, data) => _dbQuery('update', { table, pk, data }),\n"
        "    delete: (table, pk)     => _dbQuery('delete', { table, pk }),\n"
        "  },\n"
        "  mail: { send: (o) => _httpSync('POST', '/mail/send', o) },\n"
        "};\n"
    )
...
def _write_lemat_inits(project: str):
    ...
    (project_dir / "_lemat_api_init.js").write_text(_generate_js_api_init(project))
...
async def _run_cron_job(project: str, job_id: str):
    ...
    try:
        stdout, stderr, returncode, _ = await docker_runner.run_command(
            command=command,
        ...
        _log_cron_run(project, job_id, job["name"], status, returncode, output, ran_at)
...
async def startup():
    ...
    _scheduler.start()
    _reload_all_crons()
    # Build Docker runner image in background (non-blocking)
    asyncio.create_task(_init_docker_runner())

async def _init_docker_runner():
    """Try to build the Docker runner image at startup."""
    try:
        available = await docker_runner.is_docker_available()
        if available:
            built = await docker_runner.ensure_runner_image()
            if built:
                print("[lemat] Docker isolation: ACTIVE")
            else:
                print("[lemat] Docker isolation: image build failed, using subprocess fallback")
        else:
            print("[lemat] Docker isolation: Docker not available, using subprocess fallback")
    except Exception as e:
        print(f"[lemat] Docker isolation: error ({e}), using subprocess fallback")
...
def _api_keys_path(project: str) -> Path:
    return safe_path(project) / "api_keys.json"

def _load_api_keys(project: str) -> list:
    path = _api_keys_path(project)
    if path.exists():
        try:
            return json.loads(path.read_text())
        except Exception:
...
def _save_api_keys(project: str, keys: list):
    _api_keys_path(project).write_text(json.dumps(keys, indent=2))

def _validate_api_key(project: str, key: str) -> bool:
    """Check if an API key is valid for the given project."""
    keys = _load_api_keys(project)
    return any(k["key"] == key for k in keys)


@app.get("/api/projects/{project}/api-keys")
def list_api_keys(project: str):
    if not safe_path(project).exists():
        raise HTTPException(404, "Project not found")
    keys = _load_api_keys(project)
    # Return keys with masked values
    return [{"id": k["id"], "name": k["name"], "key": k["key"][:8] + "..." + k["key"][-4:],
             "created_at": k["created_at"]} for k in keys]


@app.post("/api/projects/{project}/api-keys")
def create_api_key(project: str, body: dict):
    if not safe_path(project).exists():
        raise HTTPException(404, "Project not found")
    name = (body.get("name") or "default").strip()
    key = "lm_" + uuid.uuid4().hex
    keys = _load_api_keys(project)
    entry = {
        "id": uuid.uuid4().hex[:8],
        "name": name,
    ...
    keys.append(entry)
    _save_api_keys(project, keys)
    # Return full key only on creation
    return entry


@app.delete("/api/projects/{project}/api-keys/{key_id}")
def delete_api_key(project: str, key_id: str):
    if not safe_path(project).exists():
        raise HTTPException(404, "Project not found")
    keys = _load_api_keys(project)
    before = len(keys)
...
def _list_api_files(project_dir: Path) -> list:
    """List all .py and .js files in the api/ directory (non-hidden)."""
    api_dir = project_dir / "api"
    if not api_dir.exists():
        return []
    files = []
    for f in sorted(api_dir.rglob("*")):
        if f.suffix in (".py", ".js") and not f.name.startswith("_") and f.is_file():
            files.append(f)
    return files


@app.api_route(
    "/api/projects/{project}/api/{route_path:path}",
    methods=["GET", "POST", "PUT", "DELETE", "PATCH"],
)
async def handle_user_api(project: str, route_path: str, request: Request):
    """Execute a user-defined API route."""
    project_dir = safe_path(project)
    if not project_dir.exists():
        raise HTTPException(404, "Project not found")

    # Auth: check API key, IDE JWT, or deployed context
    api_key = request.headers.get("x-api-key", "")
    user_id = getattr(request.state, "user_id", None)
    is_deployed = getattr(request.state, "deploy_project", None) is not None
    if not api_key and not user_id and not is_deployed:
        raise HTTPException(401, "API key required. Pass X-API-Key header.")
    if api_key and not _validate_api_key(project, api_key):
        raise HTTPException(403, "Invalid API key")

    # Find all API files and try each until one matches
    api_files = _list_api_files(project_dir)
    if not api_files:
        raise HTTPException(404, f"No api/ folder or files found in project")

    # Normalize route path
    full_route = "/" + route_path.strip("/") if route_path.strip("/") else "/"
    ...
    # Ensure SDK init files exist
    _write_lemat_inits(project)

    # Determine if Docker is active for path resolution
    use_docker = await docker_runner.is_docker_available() and docker_runner._image_ready

    # Try each API file — the runner returns 404 if no route matches in that file
    last_error = None
    for file_path in api_files:
        # File path: container-relative or host-absolute
        rel_path = str(file_path.relative_to(project_dir))
        file_ref = f"/project/{rel_path}" if use_docker else str(file_path.resolve())
        ...
        if ext == ".py":
            runner = "/lemat-sdk/api_runner_py.py" if use_docker else _API_RUNNER_PY
            command = ["python3", "-u", runner]
        elif ext == ".js":
            runner = "/lemat-sdk/api_runner_js.js" if use_docker else _API_RUNNER_JS
            command = ["node", runner]
        else:
            continue
        ...
        try:
            stdout, stderr, returncode, _ = await docker_runner.run_command(
                command=command,
                project_dir=str(project_dir),
                stdin_data=json.dumps(req_context).encode(),
                timeout=30.0,
            )
        except asyncio.TimeoutError:
            raise HTTPException(504, "API handler timed out (30s)")
        except Exception as e:
            last_error = str(e)
            continue
        ...
        # Find the last JSON line (SDK may print logs before it)
        lines = out.split("\n")
        json_line = None
        ...
        # If this file returned 404 (no matching route), try next file
        if response.get("status") == 404 and "No route matches" in str(response.get("body", {}).get("error", "")):
            continue
        ...
        # Use raw Response to avoid Content-Length mismatch with BaseHTTPMiddleware
        body_bytes = json.dumps(resp_body, ensure_ascii=False).encode("utf-8")
        return Response(
            content=body_bytes,
            status_code=status,
    ...
    # No file matched
    raise HTTPException(404, f"No API route matches {request.method} /{route_path}")
...
@app.post("/api/projects/{project}/move/{filepath:path}")
def move_file(project: str, filepath: str, body: MoveRequest):
    """Rename or move a file/folder. destination is relative to project root."""
    src = safe_path(project, filepath)
    if not src.exists():
        raise HTTPException(404, "Source not found")
    dst = safe_path(project, body.destination)
    if dst.exists():
        raise HTTPException(409, "Destination already exists")
    dst.parent.mkdir(parents=True, exist_ok=True)
    src.rename(dst)
    return {"message": "Moved", "from": filepath, "to": body.destination}
...
async def exec_file(project: str, filepath: str, cmd: str = None):
    ...
    # Generate SDK init files so scripts can use `from _lemat_init import lemat`
    _write_lemat_inits(project)
    ...
    async def generate():
        ...
        try:
            proc, container_name = await docker_runner.start_streaming(
                command=command,
                project_dir=str(project_dir),
            )
            active_processes[run_id] = (proc, container_name)
            ...
            while done < 2:
                try:
                    ...
                    await docker_runner.kill_process(proc, container_name)
                    break
                if data is None:
                    done += 1
                    continue
...
async def stop_run(run_id: str):
    entry = active_processes.pop(run_id, None)
    if entry:
        proc, container_name = entry
        await docker_runner.kill_process(proc, container_name)
        return {"stopped": True}
...
@app.api_route("/p/{token}/{filepath:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def serve_token_file(token: str, filepath: str, request: Request):
    ...
    # User-defined API routes via token URL
    if filepath.startswith("api/"):
        api_path = filepath[4:]
        return await handle_user_api(project, api_path, request)
...
async def serve_deployed_project(request: Request, filepath: str = ""):
    ...
    # User-defined API routes → proxy to handler
    if filepath.startswith("api/"):
        api_path = filepath[4:]  # Remove "api/"
        return await handle_user_api(project_name, api_path, request)
